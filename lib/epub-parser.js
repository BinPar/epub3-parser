...
EpubParser = (function() {
  ...
  var contentPath = '';
    ...
    if (bypassDecompress) {
      try {
        ...
        throw new Error('Error getting file ' + filename + ': ' + err.message);
      }
    } else {
  ...
  function safeAccess(supposedArray) {
    ...
    if (arguments.length < 2) {
      throw new Error('epub3-parser - open function must receive at least two arguments');
    ...
    var htmlNav = '<ul>';
      ...
      if (!bypassDecompress) {
        ...
        zip = new jszip(data.toString('binary'), {binary:true, base64: false, checkCRC32: false});
      }
