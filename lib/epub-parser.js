...
EpubParser = (function () {
  ...
  function extractText(filename) {
    ...
    if (bypassDecompress) {
      try {
        return fs.readFileSync(path.resolve(contentPath, filename), 'utf8');
      } catch (err) {
        throw new Error(
          'Error getting file extractText ' + filename + ': ' + err.message,
        );
      }
    } else {
  ...
  function extractBinary(filename) {
    if (bypassDecompress) {
      try {
        return fs.readFileSync(path.resolve(contentPath, filename));
      } catch (err) {
        throw new Error(
          'Error getting file extractBinary ' + filename + ': ' + err.message,
        );
      }
    } else {
      var file = zip.file(filename);
  ...
  function open() {
    var filename, md5hash, cb;
    if (arguments.length < 2) {
      throw new Error(
        'epub3-parser - open function must receive at least two arguments',
      );
    ...
    function parseContainer(err, containerJSON, finalCallback) {
      ...
      console.log('parsing OPF data', root);
      if (root) {
        opfDataXML = extractText(root);
      } else {
        console.log('Before call back!');
        cb(new Error('Missing root'));
      }
      ...
      parser.parseString(opfDataXML.toString(), function (err, opfJSON) {
        ...
        if (!ncxId) {
          ...
          if (ncxPath) {
            ...
            return cb(new Error('Missing ncxPath'));
          }
          ...
          parser.parseString(ncxDataXML.toString(), function (err, ncxJSON) {
            ...
            ncx =
              ncxJSON[ncxPrefix + 'ncx'] != undefined
                ? ncxJSON[ncxPrefix + 'ncx']
                : ncxJSON['ncx'];

            var navPoints =
              ncx[ncxPrefix + 'navMap'] != undefined
                ? ncx[ncxPrefix + 'navMap'][0][ncxPrefix + 'navPoint']
                : ncx['navMap'][0]['navPoint'];
    ...
    function buildLinearSpine() {
      for (var itemref in itemreflist) {
